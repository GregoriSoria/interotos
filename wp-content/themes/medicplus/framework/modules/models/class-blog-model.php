...
class Medicplus_Blog_Model {
	...
	public function set_attributes( $atts = array() ) {
		$this->uniq_id = Medicplus::make_id();
		$default = array(
			'layout'               => '',
			'style'                => '',
			'block_title'          => '',
			...
			'pagination'           => '',
			'category_filter'      => '',
			'category_filter_text' => esc_html__( 'All', 'medicplus' ),
			'title_length'         => '',
			'excerpt_length'       => '',
			'show_category'        => '',
			'show_tag'             => '',
	...
	private function get_query( $data, $paged = '') {
		...
		switch ( $data['sort_by'] ) {
			...
			case 'za_order':
				...
				break;
			case 'popular':
				$query_args['meta_key'] = 'medicplus_postview_number';
	...
	public function get_views( $echo= false ) {
		...
		if( $this->attributes['show_views'] != 'hide' ) {
			...
			$view_text = sprintf(_n('%s View', '%s Views', $viewcount, 'medicplus'), $viewcount );
			$out = sprintf( $format, esc_url($this->permalink), $view_text);
		}
		if( ! $echo ) {
			return $out;
		}
	...
	public function get_comments( $echo = false ) {
		...
		if( $this->attributes['show_comments'] != 'hide' ) {
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				$out = sprintf( $format, esc_url(get_comments_link( $this->post_id )), get_comments_number_text() );
			}
		}
		if( ! $echo ) {
			return $out;
	...
	public function get_featured_image( $thumb_type = 'large', $echo = false, $options = array() ) {
		...
		if( $this->has_thumbnail ) {
			...
			$thumb_img = sprintf('<img src="%1$s" alt="%2$s" class="img-responsive" />', MEDICPLUS_NO_IMG_URI . $no_image ,$alt);
		}
		$format_icon = Medicplus_Params::get( 'post-format-icon' );
	...
	public function get_featured_images( $thumb_type = 'large', $echo = false, $options = array() ) {
		...
		if( $this->has_thumbnail ) {
			...
			$thumb_img = sprintf('<img src="%1$s" alt="%2$s" />', MEDICPLUS_NO_IMG_URI . $no_image ,$alt);
		}
		$format_icon = Medicplus_Params::get( 'post-format-icon' );
		$post_format = get_post_format( $this->post_id );
		if( isset($format_icon[$post_format]) ) {
	...
	public function get_title( $is_limit = true, $echo = false, $options = array() ) {
		...
		if( ! $echo ) {
			return sprintf( $output, esc_attr( $title ), esc_url($this->permalink), esc_attr($options['title_class']) );
		} 
		printf( $output, esc_attr( $title ), esc_url($this->permalink), esc_attr($options['title_class']) );
	}
	/**
	 * Get the permalink
	 */
	...
	public function view_more_button( $echo = false, $post_id = '', $btn_content = '', $html_options = array() ) {
		if( empty( $btn_content ) ) {
			$btn_content = esc_html__( 'View more', 'medicplus' );
		}
	...
	public function get_thumb_sizes( $sizes, $options= array(), $theme_prefix = 'medicplus' ) {
	...
	public function get_featured_by_format( $thumb_type = 'large', &$options = array() ) {
		...
		if( $this->isVideo( $this->post_id ) && class_exists('Medicplus_Core_Video_Model')) {
			//video
			$model = new Medicplus_Core_Video_Model();
			$feature_video = $model->get_feature_video( $this->post_id, 'medicplus_feature_video' );
			if( $featured_image ) {
				$options['bg_image'][$this->post_id] = $opts['ret_attach_img'][0];
			}
			if( $feature_video && $featured_image ) {
