<?php
	//Back end
	add_filter( 'wp_edit_nav_menu_walker', 'medicplus_edit_nav_menu', 10, 2 );
	function medicplus_edit_nav_menu( $walker, $menu_id ) {
		return 'Medicplus_Nav_Menu_Edit_Custom';
	}
	
	add_action( 'wp_update_nav_menu_item', 'medicplus_update_menu', 100, 3);

	//update menu
	function medicplus_update_menu($menu_id, $menu_item_db)
	{
		
		$check = array('show-megamenu','megamenu-style','megamenu-column','choose-icon','choose-widgetarea','show-widget','tab-title','megamenu-widget-column');
	...
	if( !class_exists( 'Medicplus_Nav_Menu_Edit_Custom' ) )
	{
		class Medicplus_Nav_Menu_Edit_Custom extends Walker_Nav_Menu
		{
			/**
			...
			public function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
				...
				if ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
					$classes[] = 'pending';
					$title = sprintf( esc_html__('%s (Pending)', 'medicplus'), $item->title );
				}
				...
				<li id="menu-item-<?php echo esc_attr( $item_id ); ?>" class="<?php echo esc_attr( $itemValue ); echo ' ' . implode(' ', $classes ); ?>">
					<dl class="menu-item-bar">
						<dt class="menu-item-handle">
							...
							<span class="item-controls">
								...
								<a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" title="<?php esc_html_e('Edit Menu Item', 'medicplus' ); ?>" href="<?php
								...
								?>"><?php esc_html_e('Edit Menu Item', 'medicplus' ); ?></a>
							</span>
						</dt>
					</dl>
					...
					<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
						<?php if( 'custom' == $item->type ) : ?>
							<p class="field-url description description-wide">
								<label for="edit-menu-item-url-<?php echo esc_attr( $item_id ); ?>">
									<?php esc_html_e( 'URL', 'medicplus' ); ?><br />
						...
						<p class="description description-thin description-label slz_label_desc_on_active">
							...
							<span class='slz_default_label'><?php esc_html_e( 'Navigation Label', 'medicplus'  ); ?></span>
								<br />
						...
						<p class="description description-thin description-title">
							<label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
								<?php esc_html_e( 'Title Attribute', 'medicplus'  ); ?><br />
								<input type="text" id="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-attr-title" name="menu-item-attr-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->post_excerpt ); ?>" />
							</label>
						</p>
						<p class="field-link-target description description-thin">
							<label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
								<?php esc_html_e( 'Link Target', 'medicplus'  ); ?><br />
								<select id="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-target" name="menu-item-target[<?php echo esc_attr( $item_id ); ?>]">
									<option value="" <?php selected( $item->target, ''); ?>><?php esc_html_e('Same window or tab', 'medicplus' ); ?></option>
									<option value="_blank" <?php selected( $item->target, '_blank'); ?>><?php esc_html_e('New window or tab', 'medicplus' ); ?></option>
								</select>
							</label>
						</p>
						<p class="field-css-classes description description-thin">
							<label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
								<?php esc_html_e( 'CSS Classes (optional)' , 'medicplus' ); ?><br />
								<input type="text" id="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
							</label>
						</p>
						<p class="field-xfn description description-thin">
							<label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
								<?php esc_html_e( 'Link Relationship (XFN)', 'medicplus'  ); ?><br />
								<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
							</label>
						</p>
						<p class="field-description description description-wide">
							<label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
								<?php esc_html_e( 'Description', 'medicplus'  ); ?><br />
						...
						<div class='slz_menu_options'>
							...
							<p class="shw_hide_d0 shw_text choose-icon  shw_mega_menu ">
								<label for="edit-<?php echo esc_attr( $key ).'-'. esc_attr( $item_id ); ?>"><?php esc_html_e( 'Enter icon for menu' , 'medicplus'); ?><br>
									<input type="text"  id="edit-<?php echo esc_attr( $key ).'-'. esc_attr( $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key ) . "[". $item_id ."]";?>" value="<?php echo esc_attr( $value ); ?>" /><?php esc_html_e( 'Ex:fa-crosshairs', 'medicplus' );?>
								</label>
							</p>
							<!--use mega menu-->
							<?php 
							...
							<p class="description description-wide slz-show-megamenu slz-mega-menu-d0">
								<label for="edit-<?php echo esc_attr( $key ) . '-' . esc_attr( $item_id ); ?>">
									<input type="checkbox" value="active" id="edit-<?php echo esc_attr( $key ) . '-' .esc_attr( $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key ) . "[". esc_attr( $item_id )."]";?>" <?php echo esc_attr( $show_megamenu ); ?> /><?php esc_html_e( 'Use as Mega Menu' , 'medicplus'); ?>
								</label>
							</p>
							...
							<p class="megamenu-style description description-wide <?php echo esc_attr( $megamenu_item ); ?>">
								<label for="edit-menu-item-megamenu-style-<?php echo esc_attr( $item_id ); ?>">
									<?php esc_html_e( 'Select style for megamenu' , 'medicplus'); ?>
									<select id="edit-menu-item-megamenu-style-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-megamenu-style" name="menu-item-slz-megamenu-style[<?php echo esc_attr( $item_id ); ?>]">
										<option value="0" <?php selected( $value, '0' ); ?>><?php esc_html_e( 'Block' , 'medicplus'); ?></option>
										<option value="1" <?php selected( $value, '1' ); ?>><?php esc_html_e( 'Tab' , 'medicplus'); ?></option>
									</select>
								</label>
							</p>
							<!--Choose column for normal mega menu-->
							...
							<p class="megamenu-column description description-wide <?php echo esc_attr( $megamenu_item ); ?>">
								<label for="edit-menu-item-megamenu-column-<?php echo esc_attr( $item_id ); ?>">
									<?php esc_html_e( 'Select column number for megamenu' , 'medicplus'); ?>
									<select id="edit-menu-item-megamenu-column-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-megamenu-column" name="menu-item-slz-megamenu-column[<?php echo esc_attr( $item_id ); ?>]">
										<option value="0" <?php selected( $value, '0' ); ?>><?php esc_html_e( '1 Column' , 'medicplus'); ?></option>
										<option value="1" <?php selected( $value, '1' ); ?>><?php esc_html_e( '2 Columns' , 'medicplus'); ?></option>
										<option value="2" <?php selected( $value, '2' ); ?>><?php esc_html_e( '3 Columns' , 'medicplus'); ?></option>
										<option value="3" <?php selected( $value, '3' ); ?>><?php esc_html_e( '4 Columns' , 'medicplus'); ?></option>
									</select>
								</label>
							</p>
							...
							<p class="slz_text tab-title slz_mega_menu <?php echo esc_attr( $megamenu_item ); ?>">
								<label for="edit-<?php echo esc_attr( $key ) . '-' . esc_attr( $item_id ); ?>"><?php esc_html_e( 'Enter title  for mega menu style "Tab" ' , 'medicplus'); ?><br>
							...
							<p class="description description-wide choose-widgetarea  <?php echo esc_attr( $widget_item ) ;?>">
								<label for="edit-menu-item-megamenu-widgetarea-<?php echo esc_attr( $item_id ); ?>">
									<?php esc_html_e( 'Select Widget Area' , 'medicplus'); ?>
									<select id="edit-menu-item-megamenu-widgetarea-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-megamenu-widgetarea" name="menu-item-slz-choose-widgetarea[<?php echo esc_attr( $item_id ); ?>]">
										<option value="0"><?php esc_html_e( 'Select Widget Area' , 'medicplus'); ?></option>
										<?php
							...
							<div class="menu-item-actions description-wide submitbox">
								<?php if( 'custom' != $item->type ) : ?>
									<p class="link-to-original">
										<?php printf( esc_html__('Original: %s', 'medicplus'), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
									</p>
								<?php endif; ?>
								...
								); ?>"><?php esc_html_e('Remove', 'medicplus'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php echo add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, esc_url(admin_url( 'nav-menus.php' )) ) );
									?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e('Cancel', 'medicplus'); ?></a>
							</div>
	...
	//frontend
	if( ! function_exists( 'medicplus_show_main_menu' ) ) {
		function medicplus_show_main_menu() {
			$walker = new Medicplus_Nav_Walker;
			if ( has_nav_menu( 'main-nav' ) ) {
				wp_nav_menu( array(
					'theme_location'  => 'main-nav',
					'container'       => 'ul',
					'menu_class'      => 'nav-links nav navbar-nav medicplus-menu',
					'walker'          => $walker
				));
			}
		}
	 ...
	 * Add class active
	 */
	if( ! function_exists( 'medicplus_special_nav_class' ) ) {
		function medicplus_special_nav_class( $classes, $item ) {
			if( in_array( 'current-menu-item', $classes ) || ( in_array( 'current-menu-ancestor', $classes ) ) ) {
				$classes[] = 'active';
			}
			return $classes;
		}
		add_filter( 'nav_menu_css_class', 'medicplus_special_nav_class', 10, 2 );
	}
	/**
	 * Menu
	 */
	class Medicplus_Nav_Walker extends Walker_Nav_Menu {
		
		
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			$dropdown_align =  Medicplus::get_option('slz-dropdownmenu-align');
			...
			if ($this->mega_active == "active" ){
				...
				if( $depth == 0 ) {
					$output .= $indent.'<ul class="dropdown-menu dropdown-menu-1 medicplus-dropdown-menu-1">';
				}
				else{
					$output .= $indent.'<ul class="dropdown-menu dropdown-menu-2 medicplus-dropdown-menu-2 '.$dropdown_align.'">';
				}
			}
		}
			...
			if ($this->mega_active == "active"){
				...
				if($this->mega_style == 1){
					...
					if( $depth == 0 ) {
						...
						$item_output .= '<a href="javascript:void(0)" class="sf-with-ul"><i class="fa  '.$this->icon.'"></i>';
					}else{
						$item_output .= '<a class="link-page" ' . $attributes . '><span class="text"><i class="fa  '.$this->icon.'"></i>';
					}
