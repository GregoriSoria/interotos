...
class Medicplus_Abstract {
	...
	public function save() {
		...
		foreach( func_get_args() as $field ) {
			$new = Medicplus::get_request_param( $field, NULL );
	...
	public function gallery( $name, $images_id ) {
				...
				if( !empty($id) ){
					...
					if($image_src){
						printf('<li class="image" data-attachment_id="%s">
								...
								$id,
								$image_src[0],
								esc_html__('Delete image', 'medicplus')
							);
					}
				}
			}
		...
		printf('<a href="#" data-delete="%2$s" data-title="%3$s" data-btn-text="%4$s" class="btn-open-gallery" title="%1$s">%1$s</a>',
				esc_html__('Add gallery images', 'medicplus'),
				esc_html__('Delete image', 'medicplus'),
				esc_html__('Gallery Images', 'medicplus'),
				esc_html__('Add to gallery', 'medicplus')
			);
		echo '</p>';
	}
	/**
	...
	public function upload_attachment( $name, $attachments_ids ) {
				...
				if( !empty($id) ){
					...
					if($image_src){
						printf('<li class="image" data-attachment_id="%1$s">
								...
								esc_attr( $file_name ),
								esc_html( $type ),
								esc_html__('Remove  attachment', 'medicplus'),
								esc_html__('Remove', 'medicplus'),
								esc_url( $url )
							);
					}
				}
		...
		printf('<a href="#" data-delete="%2$s" data-title="%3$s" data-btn-text="%4$s" class="btn-open-attachment button" title="%1$s">%1$s</a>',
				esc_html__('Add Media', 'medicplus'),
				esc_html__('Remove', 'medicplus'),
				esc_html__('Attachments', 'medicplus'),
				esc_html__('Add media', 'medicplus')
			);
		echo '</p>';
	}
