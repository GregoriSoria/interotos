...
class Medicplus_Core_Com {
	...
	/**
	 * Get list display name (ỏ title if empty display name) of post typw
	 * 
	 */
	public static function get_post_display_name( $args = array(), $options = array() ) {
		...
		if( $records ) {
			foreach( $records as $row ) {
				...
				$display_name = Medicplus_Core::get_value( $option, 'display_name' );
	 ...
	 * 
	 */
	public static function get_tax_options2name( $taxonomy, $params = array() ) {
		$terms = get_terms( $taxonomy, array('orderby'=>'name', 'hierarchical'=>false, 'hide_empty' => true));
		$options = array();
		if(isset($params['empty'])) {
			$options[''] = $params['empty'];
	...
	public static function get_category2slug_array( $all_category = true, $args = array() ) {
		...
		if ( empty( self::$sw_category2slug_walker_buffer ) ) {
			...
			$category_walker = new Medicplus_Core_Category2Slug_Walker();
	 ...
	 * 
	 */
	public static function get_tax_options_id2name( $taxonomy, $params = array() ) {
		$terms = get_terms( $taxonomy, array('orderby'=>'name', 'hierarchical'=>false, 'hide_empty' => false));
		$options = array();
		if(isset($params['empty'])) {
			$options[''] = $params['empty'];
		}
 ...
 *
 */
class Medicplus_Core_Category2Slug_Walker extends Walker {
