...
class Medicplus_Core_Abstract {
	...
	public function model( $key, $def = NULL, $all = false ) {
		...
		if( $data && isset( $data[ $key ] ) ) {
			if( $all ) {
				...
				//return htmlspecialchars( $data[ $key ][0] );
				return $data[ $key ][0];
			}
		}
	...
	public function save() {
		...
		foreach( func_get_args() as $field ) {
			$new = Medicplus_Core::get_request_param( $field, NULL );
	...
	public function upload_video($name,$value='',$label, $desc = '')
	{
		...
		$output .= '<div class="upload">';
			$output .= '<div class="upload-video">';
				$output .= '<div><input name="'.esc_attr($name).'" class="textbox-url-video" type="text" value="'.esc_attr($value).'" /></div>';
	...
	public function tooltip_html( $content, $position = 'right' ) {
		echo '<a href="#" class="slzcore-tooltip" tabindex="-1" data-position="' . $position . '" data-content-as-html="true" title="' . esc_attr($content) . '">?</a>';
	}
	/**
	...
	public function gallery( $name, $images_id ) {
		echo '<div id="slzcore_gallery_container">
				...
				if( !empty($id) ){
					...
					if($image_src){
						printf('<li class="image" data-attachment_id="%s">
								...
								$id,
								$image_src[0],
								esc_html__('Delete image', 'slz-core')
							);
					}
				}
			}
		...
		echo '	</ul>';
		printf('<input type="hidden" id="slzcore_gallery_image_ids" name="%s" value="%s" />',
				 esc_attr($name),
				 esc_attr($images_id)
			);
		echo '</div>';
		...
		printf('<a href="#" data-delete="%2$s" data-title="%3$s" data-btn-text="%4$s" class="btn-open-gallery" title="%1$s">%1$s</a>',
				esc_html__('Add gallery images', 'slz-core'),
				esc_html__('Delete image', 'slz-core'),
				esc_html__('Gallery Images', 'slz-core'),
				esc_html__('Add to gallery', 'slz-core')
			);
		echo '</p>';
	}
	/**
	...
	public function upload_attachment( $name, $attachments_ids ) {
		echo '<div id="slzcore_attachment_container">
				...
				if( !empty($id) ){
					...
					if($image_src){
						printf('<li class="image" data-attachment_id="%1$s">
								...
								esc_attr( $file_name ),
								esc_html( $type ),
								esc_html__('Remove  attachment', 'slz-core'),
								esc_html__('Remove', 'slz-core'),
								esc_url( $url )
							);
					}
				}
		...
		echo '	</ul>';
		printf('<input type="hidden" id="slzcore_attachment_image_ids" name="%s" value="%s" />',
				 esc_attr($name),
		...
		printf('<a href="#" data-delete="%2$s" data-title="%3$s" data-btn-text="%4$s" class="btn-open-attachment button" title="%1$s">%1$s</a>',
				esc_html__('Add Media', 'slz-core'),
				esc_html__('Remove', 'slz-core'),
				esc_html__('Attachments', 'slz-core'),
				esc_html__('Add media', 'slz-core')
			);
		echo '</p>';
	}
	public function single_image( $name, $value, $html_options = array() ) {
		$img_src = SLZCORE_ASSET_URI . '/images/placeholder.png';
		$img_show = 'hide';
		if( ! empty( $value )) {
			$attachment_image = wp_get_attachment_image_src($value, 'full');
			$img_src = $attachment_image[0];
			$img_show = '';
		}
		$output = '<div class="screenshot '.$img_show.'" ><img class="img-responsive" src="'.esc_url( $img_src ).'"/></div>';
		$img_id_name = $name . '_id';
		if( !empty( $prefix ) ) {
			$img_id_name = $prefix . '[' . $img_id_name . ']';
		}
		$output .= '
				<input type="button" data-rel="' . $html_options['data-rel'] .'" class="button slz-btn-upload" value="'. esc_html__( 'Upload Image', 'slz-core' ) .'" />
				<input type="button" data-rel="' . $html_options['data-rel'] .'" class="button slz-btn-remove '.$img_show.'" value="'. esc_html__( 'Remove', 'slz-core' ) .'" />
				';
	...
	public function date_picker( $name, $value='', $html_options = array() ) {
		$classes = array();
		$classes[] = 'datepicker';
		...
		$html_options['class'] = join(' ' , array_unique( $classes ) );
		return $this->input_field( 'text', $name, $value, $html_options );
	}
	
	public function time_picker( $name, $value='', $html_options = array() ) {
		$classes = array();
		$classes[] = 'timepicker';
		if( isset( $html_options['class'] ) ) {
			$classes[] = $html_options['class'];
