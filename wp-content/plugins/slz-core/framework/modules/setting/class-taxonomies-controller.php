...
class Medicplus_Core_Taxonomies_Controller extends Medicplus_Core_Abstract {
	private $custom_taxonomies;
	private $taxonomy_opt = SLZCORE_TAXONOMY_CUS;

	public function __construct() {
		// Add form
		$this->custom_taxonomies = Medicplus_Core_Config::get('custom_taxonomies');
	...
	public function taxonomy_columns( $columns ) {
		...
		$new_columns['color'] = esc_html__('Color', 'slz-core');
	...
	public function display_taxonomy_column( $out_columns, $column_name, $term_id, $taxonomy = '' ) {
		...
		$color = Medicplus_Core::get_value( $data_meta, 'color' );
		$bg_color = Medicplus_Core::get_value( $data_meta, 'background_color' );
		...
		if ( $column_name == 'color' ) {
			...
			if( !empty( $custom_css ) ) {
				...
				apply_filters(SLZCORE_ADD_INLINE_CSS, $custom_css);
			}
		}
		return $out_columns;
	}
...
new Medicplus_Core_Taxonomies_Controller();