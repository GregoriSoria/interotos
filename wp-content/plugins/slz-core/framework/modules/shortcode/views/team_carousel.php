...
$post_count = $model->query->post_count;
$attr_count = $data_column = '';
if ( !empty( $post_count ) ) {
	$attr_count = 'data-count='.absint($post_count).'';
}
if ( !empty( $atts['column'] ) ) {
	$data_column = 'data-column='.absint($atts['column']).'';
}
$attr_autospeed = $attr_speed = '';
if ( !empty( $atts['auto_speed'] ) ) {
	$attr_autospeed = 'data-autospeed='.absint($atts['auto_speed']).'';
}
if ( !empty( $atts['speed'] ) ) {
	$attr_speed = 'data-speed='.absint($atts['speed']).'';
}
...
if ( !empty($count_post) && ($count_post <= $model->attributes['column']) ) {
	$custom_css .= sprintf('.%s .our-team .nav-doctor { display: none;}', $model->attributes['uniq_id'] );
}
if ( !empty($custom_css) ) {
	do_action( SLZCORE_ADD_INLINE_CSS, $custom_css );
}

// 1$ - image, 2$ - title no link, 3$ - title, 4$ - position, 5$ - social, 6$ - permalink
$html_format = '
...
';
?>
<div class="slz-shortcode sc_team_carousel <?php echo esc_attr( $block_cls ); ?>" data-carousel="<?php echo esc_attr( $model->attributes['uniq_id'] ); ?>">
	<div class="our-team">
		<div class="col-lg-12">
	    	<div class="team-wrapper owl-carousel" <?php echo esc_attr( $data_column ); ?> <?php echo esc_attr( $attr_autospeed ); ?> <?php echo esc_attr( $attr_speed ); ?> <?php echo esc_attr( $attr_count ); ?>>
				<?php $model->render_carousel( array('html_format' => $html_format) );?>
			</div>
	        <div class="nav-doctor">
	        	<a href="javascript:void(0)" class="nav-doctor-inner-left"><i class="fa fa-angle-left"></i></a>
	       		<a href="javascript:void(0)" class="nav-doctor-inner-right"><i class="fa fa-angle-right"></i></a>
	        </div>
		</div>
