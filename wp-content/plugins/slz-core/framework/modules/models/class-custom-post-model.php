<?php
class Medicplus_Core_Custom_Post_Model {
	public $post_id;
	public $cur_post;
	public $permalink;
	public $taxonomy_cat;
	...
	public function get_query( $args = array(), $data = array() ) {
		...
		// Offset start: 0
		$offset = Medicplus_Core::get_value( $data, 'offset_post', 0 );
		...
		$author_data = Medicplus_Core::get_value( $data, 'author', 0 );
	...
	private function parse_order_by( &$query_args, $data ) {
		...
		$sort_by = Medicplus_Core::get_value( $data, 'sort_by' );
		switch ( $sort_by ) {
			case 'az_order':
	...
	public function get_featured_image( $html_options = array(), $thumb_type = 'large', $echo = false ) {
		...
		$href_class = Medicplus_Core::get_value( $html_options, 'thumb_href_class' );
		$thumb_class = Medicplus_Core::get_value( $html_options, 'thumb_class', 'img-responsive' );

		$attrArr = array( 'class' => $thumb_class );
		if ( !empty($html_options['thumb_attr']) ) {
			$attrArr = array_merge( $html_options['thumb_attr'], $attrArr );
		}
		...
		if( $this->has_thumbnail ) {
			...
			$thumb_img = Medicplus_Core_Util::get_no_image( $this->attributes['thumb-size'], $this->cur_post, $thumb_type );
		}
	...
	public function get_content( $html_options = array(), $echo = false ){
		...
		$more_link_text = Medicplus_Core::get_value( $html_options, 'content_more_link_text', esc_html__('Read more', 'slz-core') );
	...
	public function get_btn_more( $html_options = array(), $echo = false ) {
		$btn_content = Medicplus_Core::get_value( $this->attributes, 'btn_content', esc_html__('More', 'slz-core') );
