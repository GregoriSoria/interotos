<?php
class Medicplus_Core_Blog_Model {
	public $post_id;
	public $title;
	...
	public function set_attributes( $atts = array() ) {
		$this->uniq_id = Medicplus_Core::make_id();
		$default = array(
			'layout'               => '',
			'style'                => '',
			'block_title'          => '',
		...
		if( empty( $data['tag_slug'] ) ) {
			list( $data['tag_list_parse'], $data['tag_slug'] ) = Medicplus_Core_Util::get_list_vc_param_group( $data, 'tag_list', 'tag_slug' );
		}
		if( empty( $data['author'] ) ) {
			list( $data['author_list_parse'], $data['author'] ) = Medicplus_Core_Util::get_list_vc_param_group( $data, 'author_list', 'author' );
		}
		// Check valid
	...
	private function get_query( $data, $paged = '') {
		...
		switch ( $data['sort_by'] ) {
			...
			case 'za_order':
				...
				break;
			case 'popular':
				$query_args['meta_key'] = SLZCORE_POST_VIEWS;
	...
	public function get_author( $echo= false ) {
		...
		if( $this->attributes['show_author'] != 'hide' ) {
			if ( is_singular() || is_multi_author() ) {
				$format = '<i class="fa fa-user"></i>'.esc_html__('Posted by', 'slz-core').'<a class="author" href="%1$s">%2$s</a>';
	...
	public function get_views( $echo= false ) {
		$out = '';
		$format = '<i class="fa fa-eye"></i><a href="%1$s">%2$s</a>';
		if( $this->attributes['show_views'] != 'hide' ) {
			$viewcount = $this->get_post_view( $this->post_id );
			$view_text = sprintf(_n('%s View', '%s Views', $viewcount, 'slz-core'), $viewcount );
			$out = sprintf( $format, $this->permalink, $view_text);
		}
		if( ! $echo ) {
			return $out;
		}
	...
	public function get_comments( $echo = false ) {
		$out = '';
		$format = '<i class="fa fa-comment"></i><a href="%1$s">%2$s</a>';
		if( $this->attributes['show_comments'] != 'hide' ) {
			if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				$out = sprintf( $format, get_comments_link( $this->post_id ), get_comments_number_text() );
			}
		}
		if( ! $echo ) {
			return $out;
	...
	public function get_category( $echo = false ) {
		...
		if( $this->attributes['show_category'] != 'hide' ) {
			...
			// Read the post meta
			$post_options = get_post_meta( $this->post_id, 'medicplus_post_options', true);
	...
	public function get_featured_image( $thumb_type = 'large', $echo = false, $options = array() ) {
		...
		if( isset( $options['image_format'] ) && $options['image_format'] ) {
		...
		if( $this->has_thumbnail ) {
			...
			$thumb_img = Medicplus_Core_Util::get_no_image( $this->attributes['thumb-size'], $this->post, $thumb_type, $options );
		}
		$format_icon = Medicplus_Core_Params::get( 'post-format-icon' );
	...
	public function get_featured_url( $thumb_type = 'large' ) {
		...
		if( empty($thumb_url)) {
			$thumb_url = Medicplus_Core_Util::get_no_image_url( $this->attributes['thumb-size'], $this->post, $thumb_type );
		}
		return $thumb_url;
	}
	...
	public function get_related_post( $limit = '', $echo = false, $options = array() ) {
		...
		$related_query = Medicplus_Core_Com::get_query_related_posts( $this->post_id, $args );
	...
	public function get_title( $is_limit = true, $echo = false, $options = array() ) {
		...
		if( ! $echo ) {
			return sprintf( $output, esc_attr( $title ), esc_attr($this->permalink), esc_attr($options['title_class'] ));
		} 
		printf( $output, esc_attr( $title ), esc_attr($this->permalink), esc_attr($options['title_class'] ));
	}
	/**
	 * Get the permalink
	 */
	...
	public function view_more_button( $echo = false, $post_id = '', $btn_content = '', $html_options = array() ) {
		if( empty( $btn_content ) ) {
			$btn_content = esc_html__( 'View more', 'slz-core' );
		}
	...
	public function get_featured_by_format( $thumb_type = 'large', &$options = array() ) {
		...
		if( ! $this->isVideo( $this->post_id ) ) {
			...
			//video
			$post_meta_video = get_post_meta( $this->post_id, 'medicplus_feature_video', true );
			$is_video_type = false;
			$iframe_video = '';
			...
			if ($is_video_type == true) {
				...
				if ($video_type == "youtube") {
					$iframe_video = '<iframe src="https://www.youtube.com/embed/'.esc_html( $post_meta_video['youtube_id'] ).'?rel=0" allowfullscreen="allowfullscreen" class="video-embed animated hide-video"></iframe>';
				} elseif ($video_type == "vimeo") {
					$iframe_video = '<iframe src="https://player.vimeo.com/video/'.esc_html( $post_meta_video['vimeo_id'] ).'?'.'" webkitallowfullscreen mozallowfullscreen allowfullscreen class="video-embed animated hide-video"></iframe>';
				}
			}
			if( $featured_image ) {
