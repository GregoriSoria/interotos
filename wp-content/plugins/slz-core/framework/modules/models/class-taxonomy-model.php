<?php
class Medicplus_Core_Taxonomy_Model {
	...
	public function init( $tax, $atts = array(), $query_args = array() ) {
		...
		if( !isset( $this->taxonomy_atts['uniq_id'] ) ) {
			$this->taxonomy_atts['uniq_id'] = 'block-'.Medicplus_Core::make_id();
		}
	}
	/**
	...
	public function get_responsive_class(){
		...
		$class = '';
		if( isset( $this->taxonomy_atts['column']) ) {
			$class = Medicplus_Core::get_value( $defaults, $this->taxonomy_atts['column'] );
		}
	...
	public function get_terms( $args = array() ) {
		...
		$offset = absint( Medicplus_Core::get_value($atts, 'offset', 0 ) );
	...
	public function get_custom_fields( $term_id ){
		...
		$option_key = SLZCORE_TAXONOMY_CUS . $term_id;
	...
	public function get_thumbnail( $html_options = array(), $thumb_type = 'large' ){
		...
		if( $thumb_id ) {
			...
			$thumb_img = sprintf('<img src="%1$s" alt="%2$s" class="img-responsive" />', SLZCORE_NO_IMG_URI . $no_image ,$alt);
		}
	...
	public function get_icon( $html_options = array() ) {
		...
		$icon = Medicplus_Core::get_value($this->custom_fields, 'icon');
		if( !empty($icon) ){
	...
	public function get_short_desc( $html_options = array() ) {
		...
		$short_desc = Medicplus_Core::get_value($this->custom_fields, 'short_description');
		if( !empty( $short_desc ) ) {
	...
	public function get_term_count( $html_options = array() ) {
		$show_count = Medicplus_Core::get_value( $this->taxonomy_atts, 'show_term_counts');
		if( $show_count == 'yes' ) {
			if( isset( $html_options['html_count'] ) && !empty( $html_options['html_count'] ) ) {
				$format = $html_options['html_count'];
			} else {
				$format = '<a href="%1$s">'.esc_html__('Total', 'slz-core').' %2$s</a>';
			}
	...
	public function get_button_link( $html_options = array() ) {
		...
		$btn_content = Medicplus_Core::get_value( $this->taxonomy_atts, 'btn_content');
	...
	private function get_thumb_size() {
		$params = Medicplus_Core_Params::get( 'block-image-size', $this->taxonomy_atts['image-size'] );
		$this->taxonomy_atts['thumb-size'] = Medicplus_Core_Util::get_thumb_size( $params, $this->taxonomy_atts );
	}
	/**
	 * Taxonomy Pagination
	 * 
